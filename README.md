...
> Gemini는 Google AI Studio 무료 등급을 사용 중입니다 — `gemini-2.5-flash`는 **하루 20회** 제한입니다. 유료 키로 전환하면 제한 없이 사용 가능합니다 ([변경 방법](#-gemini-lambda-환경변수-변경-방법-유료-api-키--모델-변경)).
...
```bash
# 서버 단위 테스트 (상태머신, 세션, 라우트) — Lambda 호출
...
전체 맥락을 보려면 **이 파일 → server → client → gemini-lambda → bedrock-lambda** 순서 권장.
